...
function hideVisibleElem() {
  var wpcraftBox2 = document.querySelector('.wpcraft-box-2');
...
function onAccordionClick(e) {
  accordions.forEach(function (accordion) {
    ...
    var ul = accordion.querySelector('ul');
    ul.style.display = '';
  });
  var accordion = e.currentTarget;
  var currentUl = accordion.querySelector('ul');
  accordion.classList.add('is-opened');
  currentUl.style.display = 'flex';
}

var initFooter = function initFooter() {
  var siteSectionsElement = document.querySelector('.page-footer__site-sections');
  var contactsElement = document.querySelector('.page-footer__contacts');
  ...
  accordions.forEach(function (acc) {
    return acc.addEventListener('click', onAccordionClick);
  });
};
...
formPhone.onkeydown = function () {
  ...
  if (curLen < old) {
    +old, _readOnlyError("old");
    return;
  }

  if (curLen === 2) formPhone.value = formPhone.value + '(';
  if (curLen === 6) formPhone.value = formPhone.value + ')-';
  if (curLen == 11) formPhone.value = formPhone.value + '-';
  if (curLen == 14) formPhone.value = formPhone.value + '-';
  if (curLen > 16) formPhone.value = formPhone.value.substring(0, formPhone.value.length - 1);
  +old, _readOnlyError("old");
}; // ---------------------------------
